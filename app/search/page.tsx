...
//       .then(({ data }) => {
//         setCities(data ?? []);
//         // Resolve city name param to city_id if provided by navbar
//         if (cityName && data) {
...
//               <h1 className="text-2xl sm:text-3xl font-bold text-[#1B4FD8] mb-1">
//                 {category
//                   ? `${["buy","sell"].includes(category) ? "Properties for Sale" : category === "rent" ? "Properties for Rent" : category === "commercial" ? "Commercial Properties" : category === "plot_land" ? "Plots & Land" : category === "project" ? "Projects" : "Properties"}`
//                   : cityName
...
//               <p className="text-gray-500 text-sm max-w-md">
//                 {cityName
//                   ? `Discover verified properties in ${cityName} and nearby areas.`
//                   : "Discover verified properties across Mumbai, Pune, Thane and Navi Mumbai."
//                 }
//               </p>
//             </div>
...
//                   setCityName(cities.find(c => c.id === e.target.value)?.name || "");
//                 }}
//                 className="text-sm text-gray-600 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 outline-none focus:border-[#1B4FD8] cursor-pointer">
//                 <option value="">All Cities</option>
//                 {cities.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
...
//                     {/* Clear filters */}
//                     <button onClick={() => {
//                       setCategory(""); setCityId(""); setCityName(""); setBeds(""); setBaths("");
//                       setMinPrice(""); setMaxPrice(""); setVerified(false); setFeatured(false);
//                     }} className="text-xs text-red-400 hover:text-red-600 font-medium">
//                       Clear all
//                     </button>
...
//           <div className="flex gap-2 flex-wrap">
//             {category && <FilterChip label={CATEGORIES.find(c => c.value === category)?.label ?? category} onRemove={() => setCategory("")} />}
//             {cityName  && <FilterChip label={cityName} onRemove={() => { setCityId(""); setCityName(""); }} />}
//             {beds     && <FilterChip label={`${beds} BHK`} onRemove={() => setBeds("")} />}
//             {verified && <FilterChip label="Verified" onRemove={() => setVerified(false)} />}
//             {featured && <FilterChip label="Featured" onRemove={() => setFeatured(false)} />}
//           </div>
...
type ViewMode = "grid" | "list";
type SortOption = "relevance" | "price_asc" | "price_desc" | "newest";
type Category = "buy" | "commercial" | "pg_coliving" | "plot_land" | "project" | "rent" | "sell";
...
function SearchPageInner() {
  ...
  // ── Read URL params BEFORE useState (used as initial values) ──
  const cityNameParam = searchParams.get("city")     ?? "";
  const latParam      = parseFloat(searchParams.get("lat")    ?? "0") || null;
  const lngParam      = parseFloat(searchParams.get("lng")    ?? "0") || null;
  const radiusParam   = parseFloat(searchParams.get("radius") ?? "5");
  ...
  const [cityId,     setCityId]     = useState(searchParams.get("city_id") ?? "");
  const [cityName,   setCityName]   = useState(cityNameParam);
  const [beds,       setBeds]       = useState(searchParams.get("beds")      ?? "");
  const [baths,      setBaths]      = useState(searchParams.get("baths")     ?? "");
  const [minPrice,   setMinPrice]   = useState(searchParams.get("min_price") ?? "");
  const [maxPrice,   setMaxPrice]   = useState(searchParams.get("max_price") ?? "");
  const [verified,   setVerified]   = useState(searchParams.get("verified")  === "true");
  const [featured,   setFeatured]   = useState(searchParams.get("featured")  === "true");
  const [coordLat,   setCoordLat]   = useState<number | null>(latParam);
  const [coordLng,   setCoordLng]   = useState<number | null>(lngParam);
  const [coordLabel, setCoordLabel] = useState(nearLabel);
  const [radius,     setRadius]     = useState(radiusParam);
  const [isNearby,   setIsNearby]   = useState(!!(latParam && lngParam));
  const [cities,     setCities]     = useState<City[]>([]);
  const [properties, setProperties] = useState<Property[]>([]);
  const [total,      setTotal]      = useState(0);
  const [loading,    setLoading]    = useState(true);
  ...
  useEffect(() => {
    supabase.from("cities").select("id,name,slug").eq("is_active", true).order("sort_order")
      .then(({ data }) => {
        setCities(data ?? []);
        // Resolve city name param to city_id if provided by navbar
        if (cityNameParam && data) {
          const match = data.find(
            c => c.name.toLowerCase() === cityNameParam.toLowerCase()
          );
          if (match) {
            setCityId(match.id);
            setCityName(match.name);
  ...
  const fetchProperties = useCallback(async (resetPage = true) => {
    ...
    // ── Nearby search (coordinates mode) ──────────────────
    if (coordLat && coordLng) {
      const { data } = await supabase.rpc("search_nearby_properties", {
        p_lat:       coordLat,
        p_lng:       coordLng,
        p_radius_km: radius,
      ...
      const mapped = (data ?? []).map((p: any) => ({ ...p }));
      setProperties(mapped);
      setTotal(data?.[0]?.total_count ?? mapped.length);
      setLoading(false);
      return;
    }

    // ── Standard search (city/keyword mode) ───────────────
    let q = supabase
      .from("properties")
      .select(`
        id, title, slug, category, property_type, price, price_unit,
    ...
    setTotal(count ?? 0);
    setLoading(false);
  }, [coordLat, coordLng, radius, category, cityId, beds, baths, minPrice, maxPrice, verified, featured, query, sort]);

  useEffect(() => { fetchProperties(true); }, [coordLat, coordLng, radius, category, cityId, beds, baths, minPrice, maxPrice, verified, featured, sort]);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-5">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-[#1B4FD8] mb-1">
                {isNearby && coordLabel
                  ? `Properties near ${coordLabel}`
                  : category
                  ? `${["buy","sell"].includes(category) ? "Properties for Sale" : category === "rent" ? "Properties for Rent" : category === "commercial" ? "Commercial Properties" : category === "plot_land" ? "Plots & Land" : category === "project" ? "Projects" : "Properties"}`
                  : cityNameParam
                  ? `Properties in ${cityNameParam}`
                  : "Property Listing"
                }
              </h1>
              <p className="text-gray-500 text-sm max-w-md">
                {isNearby && coordLabel
                  ? `Showing properties within ${radius}km of ${coordLabel}.`
                  : cityNameParam
                  ? `Discover verified properties in ${cityNameParam} and nearby areas.`
                  : "Discover verified properties across Mumbai, Pune, Thane and Navi Mumbai."
                }
              </p>
            </div>
          ...
          {isNearby && coordLabel && (
            <div className="flex items-center gap-3 flex-wrap mb-3">
              <div className="flex items-center gap-2 bg-[#EEF2FF] text-[#1B4FD8] text-xs font-semibold px-3 py-1.5 rounded-full">
              ...
              <div className="flex items-center gap-2">
                ...
                {[2, 5, 10, 20].map(r => (
                  <button key={r} onClick={() => setRadius(r)}
                    className={`text-xs font-semibold px-3 py-1 rounded-full border transition-colors ${radius === r ? "bg-[#1B4FD8] text-white border-[#1B4FD8]" : "bg-white text-gray-600 border-gray-200 hover:border-gray-300"}`}>
                    {r}km
                  </button>
                ))}
              </div>
              <button onClick={() => { setCoordLat(null); setCoordLng(null); setCoordLabel(""); setIsNearby(false); }}
                className="text-xs text-red-400 hover:text-red-600 font-medium">
                Clear location
              </button>
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-4 shadow-sm mb-6">
            <div className="flex flex-wrap gap-3 items-center">
              {/* Location / keyword search using LocationSearchBox */}
              <div className="flex-1 min-w-[200px] bg-gray-50 rounded-xl border border-gray-200 overflow-visible">
                <LocationSearchBox
                  value={coordLabel || query || ""}
                  placeholder="Locality, city, society or landmark…"
                  onLocationSelect={(result: LocationResult) => {
                    setCoordLat(result.lat);
                    setCoordLng(result.lng);
                    setCoordLabel(result.label);
                    setIsNearby(true);
                    setCityId("");
                    setCityName("");
                    setQuery("");
                  ...
                  inputClassName="py-2.5 bg-transparent"
                />
              </div>
                ...
                <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }}
                  ...
                  <div className="pt-4 mt-3 border-t border-gray-100 flex flex-wrap gap-4 items-center">
                    ...
                    <button onClick={() => {
                      setCategory(""); setCityId(""); setCityName(""); setBeds(""); setBaths("");
                      setMinPrice(""); setMaxPrice(""); setVerified(false); setFeatured(false);
                      setCoordLat(null); setCoordLng(null); setCoordLabel(""); setIsNearby(false);
                    }} className="text-xs text-red-400 hover:text-red-600 font-medium">
                      Clear all
                    </button>
                  </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-5 flex-wrap gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            {category && <FilterChip label={CATEGORIES.find(c => c.value === category)?.label ?? category} onRemove={() => setCategory("")} />}
            {cityName  && <FilterChip label={cityName} onRemove={() => { setCityId(""); setCityName(""); }} />}
            {isNearby && coordLabel && <FilterChip label={`Near ${coordLabel}`} onRemove={() => { setCoordLat(null); setCoordLng(null); setCoordLabel(""); setIsNearby(false); }} />}
            {beds     && <FilterChip label={`${beds} BHK`} onRemove={() => setBeds("")} />}
            {verified && <FilterChip label="Verified" onRemove={() => setVerified(false)} />}
            {featured && <FilterChip label="Featured" onRemove={() => setFeatured(false)} />}
          </div>
