...
const FEATURES = [
  {
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.6} className="w-6 h-6 sm:w-7 sm:h-7">
        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
      </svg>
    ),
    title: "100% Verified",
  ...
  return (
    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ── Cards Grid ─────────────────────────────────────── */}
        {/* Mobile: 2-up grid with smaller card padding/type (see FeatureCard).
            sm and lg breakpoints are unchanged from before. */}
        <motion.div
...
function FeatureCard({
  ...
  return (
    <motion.div
      ...
      className="group relative bg-white rounded-2xl p-4 sm:p-8 flex flex-col items-center text-center cursor-default"
      style={{
        boxShadow: "0 2px 16px -4px rgba(0,0,0,0.07)",
      }}
    >
      ...
      <motion.div
        whileHover={{ scale: 1.08 }}
        transition={{ duration: 0.25 }}
        className="w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center mb-3 sm:mb-6 text-[#2EAE88]"
        style={{ background: "rgba(46, 174, 136, 0.10)" }}
      >
        {feature.icon}
      </motion.div>
      ...
      {/* Title */}
      <h3 className="text-gray-900 text-sm sm:text-[1.05rem] font-bold leading-snug mb-1.5 sm:mb-3">
        {feature.title}
      </h3>
      ...
      {/* Bottom accent line — appears on hover */}
      <motion.div
        className="absolute bottom-0 left-4 right-4 sm:left-6 sm:right-6 h-0.5 rounded-full bg-[#2EAE88] origin-left"
        initial={{ scaleX: 0 }}
        whileHover={{ scaleX: 1 }}
        transition={{ duration: 0.35, ease: [0.22, 1, 0.36, 1] }}
      />
