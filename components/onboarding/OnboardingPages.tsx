  ...
  return (
    ...
    >
      <div className="w-full max-w-2xl">
        {/* Logo */}
        <div className="flex items-baseline mb-10">
          <span className="text-xl font-bold text-[#1B4FD8]">PropertyLink</span>
          <span className="text-xl font-bold text-[#2EAE88]">Reality</span>
        </div>
...
// STEP 2: RERA Number
// app/onboarding/step-2/page.tsx
// Only saves rera_number to agent_profiles.
// Document upload (Aadhaar, PAN, RERA cert, office proof) is
// handled in /agent/verification after onboarding is complete.
// ══════════════════════════════════════════════════════════════
export function OnboardingStep2Page() {
  const router   = useRouter();
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!reraNumber.trim()) {
      setError("Please enter your RERA number, or skip this step.");
      return;
    }
    setLoading(true);
    setError("");
  ...
  return (
    <OnboardingLayout
      step={2}
      title="Your RERA Number"
      subtitle="Add your RERA registration number. You'll upload supporting documents after setup."
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="flex items-center gap-3 bg-[#EEF2FF] border border-[#1B4FD8]/20 rounded-2xl p-4">
          ...
          <div>
            ...
            <p className="text-gray-500 text-xs mt-0.5 leading-relaxed">
              Complete verification after setup to get a badge on your profile and unlock 3x more leads.
            </p>
          </div>
        </div>
        ...
        <OField label="RERA Registration Number">
          ...
          <p className="text-xs text-gray-400 mt-1.5">
            MahaRERA, Karnataka RERA or your state's registration number.
          </p>
        </OField>

        {/* What happens next */}
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
          <p className="text-amber-800 font-semibold text-xs mb-2">📄 Documents needed after setup:</p>
          <ul className="space-y-1">
            {[
              "Aadhaar card (front & back)",
              "PAN card",
              "RERA certificate",
              "Office proof (optional)",
            ].map(d => (
              <li key={d} className="flex items-center gap-2 text-amber-700 text-xs">
                <svg className="w-3 h-3 text-amber-500 flex-shrink-0" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24">
          ...
          <p className="text-amber-600 text-[10px] mt-2">
            You'll upload these in the Verification Center after completing setup.
          </p>
        </div>
      ...
      >
        ...
        <div className="flex flex-col gap-3">
          ...
          <motion.button
            ...
            className="w-full bg-[#2EAE88] hover:bg-[#28996f] text-white font-semibold text-sm py-4 rounded-xl transition-colors flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
            </svg>
            Complete Verification (Get Verified Badge)
          </motion.button>
          <button
            onClick={() => router.push("/agent/dashboard")}
            className="w-full border border-gray-200 hover:border-gray-300 text-gray-700 font-medium text-sm py-4 rounded-xl transition-colors"
