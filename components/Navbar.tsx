...
//   { label: "Plot / Land", href: "/search?category=plot_land"  },
//   { label: "Mortgage",    href: "/mortgage"                   },
// ];
...
//   // Navigate to search filtered by city name
//   const handleCitySelect = useCallback((city: string) => {
//     setLocationQuery(city);
//     setLocationOpen(false);
...
//         setLocationQuery("Current Location");
//         setLocationOpen(false);
//         // Navigate to search — backend will use geolocation in future
//         router.push("/search");
//       },
//       () => {}
//     );
...
//   // Builds URL from locationQuery + activeType and navigates to /search
//   function handleNavSearch() {
//     const params = new URLSearchParams();
//     const q = locationQuery.trim();
//     if (q && q !== "Current Location") {
//       // Check if it matches a known city name (case-insensitive)
//       const knownCity = POPULAR_CITIES.find(
//         c => c.name.toLowerCase() === q.toLowerCase()
//       );
//       if (knownCity) {
//         params.set("city", knownCity.name);
//       } else {
//         // Treat as keyword search (society/project/landmark)
//         params.set("q", q);
//       }
//     }
//     // Map activeType to category param
...
//   // ── Post Property click logic ─────────────────────────────
//   // BUG FIX 1: Previously sent user to /register?intent=agent
//   // which middleware blocked for logged-in users.
//   // Now:
//   //   - Not logged in → /login
//   //   - Agent, onboarding done → /agent/listings/new
//   //   - Agent, onboarding incomplete → /onboarding/step-N
//   //   - Regular user → /register?intent=agent
//   //     (middleware now ALLOWS this for logged-in role=user)
//   function handlePostProperty(e: React.MouseEvent) {
//     e.preventDefault();
//     if (!authUser) {
//       router.push("/login?redirect=/post-property");
...
//         const nextStep = Math.max(1, authUser.onboardingStep + 1);
//         router.push(`/onboarding/step-${nextStep}`);
//       }
//     } else {
//       // Regular user wants to list → take them to agent registration
//       // Middleware now permits /register?intent=agent for logged-in role=user
//       router.push("/register?intent=agent");
//     }
//   }
...
//                 className="flex-shrink-0 bg-[#1B4FD8] hover:bg-[#1640b8] text-white p-2.5 rounded-full m-1 transition-colors">
//                 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                   <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
//                 </svg>
//               </button>
...
//                         const knownCity = POPULAR_CITIES.find(
//                           c => c.name.toLowerCase() === mobileQuery.trim().toLowerCase()
//                         );
//                         router.push(
//                           knownCity
//                             ? `/search?city=${encodeURIComponent(knownCity.name)}`
//                             : `/search?q=${encodeURIComponent(mobileQuery.trim())}`
//                         );
...
//                       setMobileQuery("");
//                     }}
//                     className="flex-shrink-0 bg-[#1B4FD8] text-white p-3 rounded-full m-1">
//                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
//                       <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
//                     </svg>
//                   </button>
...
export default function Navbar() {
  ...
  // Location selected from LocationSearchBox (GPS / Mapbox / Map picker)
  ...
  function handleNavSearch() {
    ...
    if (locationCoords) {
      // Location was selected via GPS / Mapbox / map picker — use coordinates
      params.set("lat",    locationCoords.lat.toString());
      params.set("lng",    locationCoords.lng.toString());
      params.set("radius", "5");
      if (locationLabel) params.set("near", locationLabel);
    } else if (locationLabel.trim()) {
      // Fallback: text-based city/keyword
      const knownCity = POPULAR_CITIES.find(
        c => c.name.toLowerCase() === locationLabel.trim().toLowerCase()
      );
      if (knownCity) {
        params.set("city", knownCity.name);
      } else {
        params.set("q", locationLabel.trim());
      }
    }

    const catMap: Record<string, string> = {
      "Buy": "buy", "Sell": "sell", "Rent": "rent",
      "Commercial": "commercial", "Plots/Land": "plot_land", "Projects": "project",
    };
    const cat = catMap[activeType];
    if (cat) params.set("category", cat);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16 lg:h-[68px] gap-4">
            ...
            <div className="hidden lg:flex flex-1 max-w-xl items-center bg-white rounded-full shadow-md overflow-visible relative">
              ...
              {/* Location input — GPS + Mapbox autocomplete + Map picker */}
          ...
          <AnimatePresence>
            {mobileSearchOpen && (
              <motion.div
                ...
                className="lg:hidden overflow-hidden pb-3 "
              >
                <div className={`bg-white rounded-full shadow-md overflow-visible ${!scrolled && "ring-1 ring-gray-200"}`}>
                  <LocationSearchBox
                    value={mobileQuery}
                    placeholder="City, locality or landmark…"
                    onLocationSelect={(result) => {
                      setMobileQuery(result.label);
                      const params = new URLSearchParams();
                      params.set("lat",    result.lat.toString());
                      params.set("lng",    result.lng.toString());
                      params.set("radius", "5");
                      params.set("near",   result.label);
                      router.push(`/search?${params.toString()}`);
                      setMobileSearchOpen(false);
                      setMobileQuery("");
                    }}
                    onClear={() => setMobileQuery("")}
                    inputClassName="px-4 py-3 rounded-full"
                  />
                </div>
              </motion.div>
            )}
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, height: 0 }}
              transition={{ duration: 0.25, ease: "easeOut" }}
              className={`lg:hidden border-t z-200 overflow-hidden ${scrolled ? "bg-[#1B4FD8] border-white/10" : "bg-white border-gray-100"}`}
            >
              <div className="px-4 py-4 space-y-1">
                {NAV_LINKS.map((l) => (
                  <Link
